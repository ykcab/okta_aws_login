...
def okta_password_login(username,password,idp_entry_url):
    ...
    if "Sign in failed!" in response.text:
        ...
        response = okta_app_mfa_login(response,"Okta Verify")
    elif "Enter your Google Authenticator code" in response.text:
        response = okta_app_mfa_login(response,"Google Authenticator")
    return response

def okta_app_mfa_login(password_login_response,app):
